...
router.put('/done/:thingId', function (req, res, next){
    ...
    mongoModel.togglePBEDone(_thingId, (err, rslt)=>{
       if(err){
           console.log(err);
           return res.status(500).json({"error":"No se pudo actualizar el PBE"});
       }
...
router.delete('/delete/:thingId', function(req, res, next){
    ...
    mongoModel.deleteById(_thingId, (err, result)=>{
        if(err){
            return res.status(500).json({"errro":"No se pudo Eliminar dato"});
        }
        return res.status(200).json(result);
    }); //deleteById
